...
axios.post(url,JSON.stringify(data) ,{
    ...
    console.log(text);
    if(response.data.hasOwnProperty("elements")){

        response.data.elements[0].errors.forEach(ele=>{alert(`word:${ele.word}\nposition:${ele.position}\nsuggestions:${ele.suggestions.map(suggestion=>{return `\n${suggestion}`})}\n`)})
    }else{
...
<div className="mb-3" >
  ...
  <button type="button" className="btn btn-dark my-3 mx-3 " onClick={handleLoCase}> convert to lowercase</button>
  <button type="button" className="btn btn-dark my-3 mx-3 " onClick={spellCheck}> Check for spell</button>

</div>
<div className="container my-3">
    <h1>your text summary</h1>
    <p>number of characters in above paragraph is {text.length} </p>
